...
def paramRedux(dataset=""):
    ...
    df=convertParams.convert(df, dataset)
    df=dropcolumns(df, dataset) #Remove columns deemed to be irrelevant

    # df=dataFrameTools.dropData(df, dataset)  #Remove columns and rows that only contain N/A's or a single value
    ...
    dtypedf.to_csv("../Data/" + dataset + "BIKED_datatypes.csv",header=["type"])
    df.to_csv(Path("../Data/" + dataset + "BIKED_reduced.csv"))
    print("Full Parameter Space Reduction completed in %s seconds" % (time.time() - start_time))    

def dropcolumns(df, dataset=""):
    if dataset=="":
        start_time=time.time()
        ...
        return df
    if dataset!="":
        params=pd.read_csv("../Dataset Definitions/" + dataset + ".csv", index_col=0).index
        reddf=df[params]
        return reddf
    
def fixunits(df, dataset): #Convert units in inches to mm and remove the unit bool column
    if dataset=="":
